...
export default class DataCheck extends Component {
  ...
  componentDidMount() {
    this.initialDataList({ pageNum: 1, pageSize: 20 }, [])
  }
  // 初始化列表数据
  initialDataList = (params, conditions) => {

    this.getDataList(params, conditions);
  }
  ...
  // filterValues = ['1', '2', ...] 
  getDataList = (params, filterValues = []) => {
    let { dataSource } = this.state;
    const { pageNum, pageSize } = params ? params : { pageNum: 1, pageSize: 100 };
    const xhr = new XMLHttpRequest()
    xhr.open("GET", `http://39.108.49.176:7321/disabilityInfoPast/selectAll?pageNum=${pageNum}&pageSize=${pageSize}`, true)
    xhr.onreadystatechange = () => {
      if (xhr.readyState === xhr.DONE) {
        dataSource = JSON.parse(xhr.responseText).records;
        this.setState({
          dataSource
        })
  ...
  // },
    ...
    const { dataSource, nTitle } = this.state;
